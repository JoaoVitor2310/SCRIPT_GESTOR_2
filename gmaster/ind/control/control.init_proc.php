     ...
     if(isset($_POST['captcha'])){
         ...
         if($captcha == $_SESSION['captcha']){
             ...
             if($dados->nome != "" && $dados->email != "" && $dados->wpp != "" && $dados->ddi != "" && $dados->id_plano != ""){
                     ...
                     if($plano){
                         ...
                         if($mail_existe){
                            ...
                            if($inser != '0'){
                               ...
                               $link = "https://cliente.gestormaster.top/{$areaCli->slug_area}?token={$token}&plano={$dados->id_plano}";
                               
                               
                               // adicioar email/whats na fila
                                $ar1  = array('{nome_cliente}','{senha}','{link_panel}');
                                $ar2  = array($dados->nome,$senha,"https://cliente.gestormaster.top/{$areaCli->slug_area}");
                                ...
                                $whatsapi_class->fila($phone,$text,$user->id,$device_id,$device->api,$codigo,$inser->id,"novo_user");
                                  ...
                                  $whatsapi_class->fila($phone2,$text,$user->id,'ZAPI','ZAPI','0000','0000',"novo_user_dono");
                             ...
                             $link = "https://cliente.gestormaster.top/{$areaCli->slug_area}?login={$dados->email}&plano={$dados->id_plano}";
                             echo json_encode(['erro' => false, 'link' => $link]);
                             die;
                         }
              ...
              echo json_encode(['erro' => true, 'msg' => 'É necessário que visite https://gestormaster.top/gmaster/p/'.str_replace('=','',base64_encode($dados->id_plano)).' para fazer este checkout']);
              die;
         }
     }else{
         echo json_encode(['erro' => true, 'msg' => 'É necessário que visite https://gestormaster.top/gmaster/p/'.str_replace('=','',base64_encode($dados->id_plano)).' para fazer este checkout']);
