   ...
   <body translate="no">
    	...
    	<?php }else{ ?>
		 ...
		 <?php } ?>
		  ...
		  function mudaDDI(ddi,country){
			  ...
			  $("#ddi").val(ddi);
			  $("#dropDownDDI").html('<img class="img_btn" src="https://<?= $gestor_class->get_options("dominio");?>/img/country/'+country+'.png" /> +'+ddi);
		  }
		</script>


	  <script src="https://<?= $gestor_class->get_options("dominio");?>/painel/js/jquery.js"></script>
	  ...
	  <script>
	   function cancel_inscricao(idcli){
	       ...
	       $.post('https://<?= $gestor_class->get_options("dominio");?>/gmaster/l/',{id:idcli,mail:mail,zap:zap},function(data){
	           if(data == '1'){
	               $("#body_center").html('<img src="https://<?= $gestor_class->get_options("dominio");?>/template_mail/template_1/img/check-icone-scaled.png" width="200" /><br /><p>Pronto! Agora você não vai mais receber mensagens.</p>');
	           }else{
	                $("#body_center").html('<img src="https://<?= $gestor_class->get_options("dominio");?>/template_mail/template_1/img/close.png" width="200" /><br /><p>Desculpe minha incompetência, mas não consegui fazer isso agora. Tente mais tarde.</p>');
