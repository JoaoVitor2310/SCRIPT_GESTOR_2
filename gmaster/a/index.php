            ...
            if($are_cli){
                ...
                if($are_cli->situ_area == 1){
                    ...
                    if(isset($explode_url[1])){
                        if($explode_url[1] == 'c'){
                           header('LOCATION: https://cliente.'.$gestor_class->get_options("dominio").'/'.$are_cli->slug_area.'?create');
                           exit();
                        }
                    }

                    header('LOCATION: https://cliente.'.$gestor_class->get_options("dominio").'/'.$are_cli->slug_area);
