...
if (isset($_REQUEST['dados'])) {
  ...
  if ($planouser->gateways == 1) {
    ...
    if ($dados->metodo = "MP" || $dados->metodo = "mp" && $mp_credenciais) {
      ...
      $payment = $gateways->pagamento_mp($mp_credenciais->client_id, $mp_credenciais->client_secret, $ref, $dados->plano, $valor, "https://cliente.gestormaster.top/");

      // echo '{"erro":true,"payment":"' . $payment . '"}'; // Debug
      // return;
