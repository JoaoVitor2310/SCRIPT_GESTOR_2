<?php 
 ...
 class Xtream extends Conn
 {
      ...
      public function getAllPackegs($cms,$username,$password,$trial=0){
       ...
       $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "http://api-panels.gestormaster.top/apps/xtreamUI/?chave=Fkw3MYiK0ya9ZCl0nZ9nZI4t7d5ZDT&panel={$cms}&username={$username}&password={$password}&trial={$trial}&action=getpackages",
      ...
      public function getAllClients($cms,$username,$password){
          ...
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_URL, "https://api-panels.gestormaster.top/apps/xtreamUI/?chave=Fkw3MYiK0ya9ZCl0nZ9nZI4t7d5ZDT&action=listusers&panel={$cms}&username={$username}&password={$password}");
      ...
      public function gerateste($cms,$username,$password,$package){
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "http://api-panels.gestormaster.top/apps/xtreamUI/?chave=Fkw3MYiK0ya9ZCl0nZ9nZI4t7d5ZDT&panel={$cms}&username={$username}&password={$password}&member_id=0&package={$package}&trial=1&action=createuser",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
       ...
       public function usercreate($cms,$username,$password,$dados){
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "http://api-panels.gestormaster.top/apps/xtreamUI/?chave=Fkw3MYiK0ya9ZCl0nZ9nZI4t7d5ZDT&action=createuser&trial=0&panel={$cms}&username={$username}&password={$password}&package={$dados->package}&usuario={$dados->usuario}&senha={$dados->senha}&member_id=",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
      ...
      public function renewuser($cms,$username,$password,$dados){
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://api-panels.gestormaster.top/apps/xtreamUI/?chave=Fkw3MYiK0ya9ZCl0nZ9nZI4t7d5ZDT&action=renew&panel={$cms}&username={$username}&password={$password}&package={$dados->package}&id_user={$dados->id_user}&user={$dados->usuario}&pass={$dados->senha}&member_id=");
        curl_setopt($ch, CURLOPT_USERAGENT,'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/32.0.1700.107 Chrome/32.0.1700.107 Safari/537.36');
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
