<?php
 ...
 class Revenda extends Conn
 {
      ...
      public function list_revendas($user)
      {
        ...
        $fetch = $query->fetchAll(PDO::FETCH_OBJ); // Busca todos os revendas
        if(count($fetch)>0){
          return $this->pdo->query("SELECT * FROM `user` WHERE id_rev='$user' ORDER BY STR_TO_DATE(vencimento, '%d/%m/%Y') "); // Se tiver mais de um, busca de maneirar ordenada
        }else{
          return false;
        }
      }
      
      public function qtd_creditos($revId)
      {
        $query = $this->pdo->query("SELECT qtd FROM `creditos_rev` WHERE id_user = '$revId'");
