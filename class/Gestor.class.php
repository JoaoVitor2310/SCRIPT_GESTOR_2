<?php
 ...
 class Gestor extends Conn
 {
      ...
      public function update_settings($name,$value){
          if($this->pdo->query("UPDATE `options_system` SET value='{$value}' WHERE name='{$name}' ")){
              return true;
          }else{
              return false;
          }
      ...
      public function get_options($name=""){
            ...
            $query = $this->pdo->query("SELECT * FROM `options_system`");
            $fetch = $query->fetchAll(PDO::FETCH_OBJ);
            if(count($fetch)>0){

              $query = $this->pdo->query("SELECT * FROM `options_system` ORDER BY id DESC ");
              $fetch = $query->fetchAll(PDO::FETCH_OBJ);
