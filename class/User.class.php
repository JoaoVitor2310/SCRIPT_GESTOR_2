...
class User extends Conn
{
  ...
  public function create($dados, $vencimento = '00/00/0000', $idp = 0, $id_rev = 0, $indicado = 0, $af = 0)
  {
    ...
    $query->bindValue(':lancar_finan', '1');
    $query->bindValue(':dark', '0');
    $query->bindValue(':somente_finan', '0');
    $query->bindValue(':gera_fat_cli', '1');
    ...
    if ($query->execute()) {
      ...
      $query2 = $this->pdo->prepare("INSERT INTO `creditos_rev` (`id_user`, `qtd`) VALUES (:lastInsertId, '0') ");
      $query2->bindValue(':lastInsertId', $lastInsertId);
