<?php
  ...
  if(isset($_SESSION['SESSION_USER']) && isset($_POST)){
    ...
    if($creditos<$minimo){
      echo '{"erro":true,"msg":"O mínimo é de 6 créditos","redirect":false}';
      die;
    }
    ...
    if($tipo_pay == "mp"){ // PIX 
      ...
      if($ref){
        ...
        $link = $mp->payment($ref,"Compra de {$creditos} creditos - Gestor Master",(float)$revenda->convertMoney(1,$plano_user->valor));
