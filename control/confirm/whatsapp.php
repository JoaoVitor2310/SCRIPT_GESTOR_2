      ...
      if(isset($_POST['confirm'])){
         ...
         if($dados_user->verificadozap == 0){
            ...
            if($phone>10){
                ...
                $json   = file_get_contents("http://api-zapi.gestormaster.top:3000/check?device={$device}&num={$phone}");
                    ...
                    if (json_last_error() === 0) {
                        ...
                        if($dados->exists->status == "200"){
                            if(isset($dados->exists->jid)){
                                ...
                                $msg  = urlencode("Oi ".explode(' ',$dados_user->nome)[0]."! Tudo bem? 💜\n\nPara confirmar seu número do Whatsapp na Gestor Master, utilize o código:\n\n*{$code}*");
                                $send = file_get_contents("http://api-zapi.gestormaster.top:3000/send?device={$device}&num={$num}&msg={$msg}");
