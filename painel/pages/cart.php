...
if (isset($_SESSION['pre_cadastro'])) {
  echo '<script>location.href="https://' . $gestor_class->get_options("dominio") . '/painel/pagamentos";</script>';
}
...
if ($id_revendedor !== '0') { // Revenda, não é possível comprar créditos direto pelo site, fala pelo wpp e o superior põe os créditos. TESTEE
  $isRevenda = true;
  // echo '<script>alert("Você é revenda.");</script>';
}
...
<main class="page-content">
  ...
  <div class="">
    ...
    <div
      ...
      <div class="col-md-12">
        <h1 class="h2 h2-cell">Planos Gestor Master</h1>
      </div>
      ...
      <div class="col-md-12">
        <div class="row">
          ...
          if($isRevenda){
            echo '<h3>Você é revendedor e não pode renovar por aqui, entre em contato com o seu superior.</h3>';
          }else{
if ($planos_gestor) {
                  ...
                  <div class="card-footer">
                    <?php if (isset($plano_usergestor->limit_cli)) { ?>
                      <?php if ($plano_usergestor->limit_cli > $plano->limit_cli) { ?>
                        <button id="button_<?= $plano->id; ?>" onclick="downgrade_plano(<?= $plano->id; ?>);"
                          style="width:100%;"
                          class="<?php if ($plano->popular == 1) {
                            echo "active";
                          } ?> btn btn-outline-primary button_pay">Contratar</button>
                      <?php } else { ?>
                        <button id="button_<?= $plano->id; ?>" onclick="checkout(<?= $plano->id; ?>);" style="width:100%;"
                          class="<?php if ($plano->popular == 1) {
                            echo "active";
                          } ?> btn btn-outline-primary button_pay">Contratar</button>
                      <?php }
                    ...
                    <?php } ?>
                    <!-- <small><a href="https://gestormaster.top/faq?wpp&text=" target="_blank" > <i class="fa fa-whatsapp"></i> Tire suas dúvidas</a></small> -->
                  </div>
                </div>
              </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="Titutlo_modal_payment">Ta quase lá
          <?= explode(' ', $user->nome)[0]; ?>
        </h5>
...
<!--  Modal downgrade -->
<div class="modal fade" id="modal_downgrade" tabindex="-1" role="dialog" aria-labelledby="Titutlo_modal_downgrade"
  aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" onclick="location.href='../control/control.export_clientes.php';"
          class="btn btn-outline-info" name="btn_export">Exportar clientes</button>
        <button type="button" onclick="check_down();" id="btn_down_confirm" class="btn btn-outline-success"
          name="button">Continuar <span id="count_sec"></span> </button>
      </div>
