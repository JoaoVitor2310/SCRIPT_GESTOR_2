     ...
     if($api_painel_info){
         ...
         if($plano_user->limit_teste > 0){
             if($vencimento == "vencido"){
                 echo json_encode(["erro" => true, "msg" => "Painel Gestor Master vencido"]);
                 die;
             }
         }else{
         ...
         if($api_painel_class->verific_mail($_REQUEST['email'],$api_painel_info->id_user,$_REQUEST['whatsapp'])){
          ...
          //   echo $teste;die;
             ...
             if($json->erro == false){
                 ...
                 $insert_historic->nota = 'Teste Gerado com Gestor Master';
                 ...
                 if($api_painel_info->template_zap == ""){
                     $template_zap = file_get_contents('https://gestormaster.top/painel/preview/modelo-teste-whatsapp.txt');
                 }else{
                 ...
                 if(isset($_REQUEST['bot'])){
                  ...
                  if($api_painel_info->template_mail == ""){
                      $template_mail = file_get_contents('https://gestormaster.top/painel/preview/modelo-teste-mail.txt');
                  }else{
                ...
                if(is_file('../cron/sends_api/send_'.$fila_lock->api.'.php')){  
                   ...
                   if($api_painel_info->receber_aviso == 1){
                       ...
                       $url = "http://api-zapi.gestormaster.top:3000/send?device=c7208f0067a370197508&num={$phone_send}&msg={$msg_send}";
